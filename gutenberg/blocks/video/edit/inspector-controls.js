 ...
 */
import { BlockAlignmentToolbar, InspectorControls, MediaUpload } from '@wordpress/block-editor';
import {
	BaseControl,
	Button,
	ExternalLink,
	...
	return (
		<InspectorControls>
			<PanelBody>
				...
				{ /* Select Videos */ }
				{ type === 'video' && (
					<BaseControl id={ __( 'Select Video', 'ghostkit' ) } label={ __( 'Select Video', 'ghostkit' ) }>
				...
				{ type === 'video' && ( videoMp4 || videoOgv || videoWebm ) ? (
					<BaseControl id={ __( 'Preview', 'ghostkit' ) } label={ __( 'Preview', 'ghostkit' ) }>
			...
			<PanelBody title={ __( 'Settings', 'ghostkit' ) }>
				...
				{ hasClass( className, 'is-style-icon-only' ) ? (
					<BaseControl id={ __( 'Icon Align', 'ghostkit' ) } label={ __( 'Icon Align', 'ghostkit' ) }>
						<div>
