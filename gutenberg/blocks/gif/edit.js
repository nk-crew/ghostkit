...
import { Button, ExternalLink, PanelBody, Placeholder, Spinner, TextareaControl, TextControl } from '@wordpress/components';
...
export default function BlockEdit( props ) {
	...
	function fetch() {
		...
		request.then( ( result ) => {
			...
			if ( result.meta && result.meta.status && result.meta.status === 200 ) {
				...
				const newResults = typeof result.data.images !== 'undefined' ? [ result.data ] : result.data;
	...
	function selectGiphy( giphy ) {
        ...
        ! new RegExp( ` ${ images[ sizeName ].width }w` ).test( newSrcset )
				...
				newSrcset += `${ newSrcset ? ', ' : '' }${ images[ sizeName ].url } ${ images[ sizeName ].width }w`;
			}
		} );
