...
const vendorFiles = [
  ...
  {
    source: 'node_modules/gmaps/gmaps.min.js',
    destination: 'assets/vendor/gmaps/gmaps.min.js',
  },
  {
    source: 'node_modules/gmaps/gmaps.min.js.map',
    destination: 'assets/vendor/gmaps/gmaps.min.js.map',
  },
  {
      ...
      patterns: [
        {
          ...
          transform(content, absoluteFrom) {
            ...
            if (path.basename(absoluteFrom) === 'block.json') {
              ...
              ['viewScript', 'script', 'editorScript'].forEach((key) => {
      ...
      cacheGroups: {
        ...
        style: {
          ...
          name(_, chunks, cacheGroupKey) {
            const chunkName = chunks[chunks.length > 1 ? 1 : 0].name;
            let cssOutput = `${path.dirname(chunkName)}/${cacheGroupKey}-${path.basename(
              chunkName
            )}`;
            const foundingChunk = chunkName.split(path.win32.sep).join(path.posix.sep);
