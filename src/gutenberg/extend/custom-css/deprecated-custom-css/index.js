...
function CustomCSSComponent(props) {
  ...
  const hasNewCustomCSSSupport = hasBlockSupport(name, ['ghostkit', 'customCSS']);

  useEffect(() => {
    // Migration to new styles attribute.
    if (hasNewCustomCSSSupport && ghostkitCustomCSS) {
      const ghostkitData = cloneDeep(attributes?.ghostkit || {});

      if (!ghostkitData?.styles?.custom) {
      ...
      // Clean old attribute.
      setAttributes({ ghostkitCustomCSS: '' });
    }
  }, []);
