...
export default class BlockEdit extends Component {
    ...
    selectGiphy( giphy ) {
        ...
        const sizes = [ 'preview_gif', 'downsized_small', 'downsized_medium', 'downsized_large', 'downsized' ];
        let srcset = '';

        sizes.forEach( ( sizeName ) => {
            if ( images[ sizeName ] && images[ sizeName ].url && ! new RegExp( ` ${ images[ sizeName ].width }w` ).test( srcset ) ) {
    ...
    parseSearch() {
        ...
        const match = searchText.match( /http[s]?:\/\/media.giphy.com\/media\/([A-Za-z0-9\-.]+)\/giphy.gif/ );
